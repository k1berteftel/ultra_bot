...
async def start_getter(event_from_user: User, dialog_manager: DialogManager, **kwargs):
    ...
    if not user.last_generate or user.last_generate < datetime.datetime.now() - datetime.timedelta(days=1):
        free = True
    bonus_text = "\n🎁<b>Бонус</b>: Каждый день тебе доступна 1 бесплатная генерация с текстом!" if free else ""
    text = (f'<b>Добро пожаловать в Flex GPT 👋🏻 </b>\n\n'
            f'<b>💬 Умный диалог</b> — задавай вопросы, ищи идеи.\n'
            f'<b>📷 Генерация фото</b> — пиши промпт - и я его выполню.\n'
            f'<b>🎬 Создание видео</b> — преврати фото в короткое и яркое видео.'
...
async def get_image_text(msg: Message, widget: ManagedTextInput, dialog_manager: DialogManager, text: str):
    ...
    if not result:
        ...
        await dialog_manager.switch_to(startSG.start)
        return
    text = f'<b>✅Готово</b>\n<b>Промпт:</b>\n<code>{text}</code>\n\n<a href="https://t.me/flexgptrobot">Бот для генерации</a>'
    message = await msg.answer_photo(
        photo=result,
        caption=text
    )
